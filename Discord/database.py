...
class DataBase:
    ...
    def file_metadata(
        ...
        self.open_connection()
        self.c.execute(
            "SELECT COUNT(file) FROM files WHERE file_name = ? \
                AND uploader =?",
            (
                file_name,
                uploader,
        ...
        if not self.c.fetchall()[0][0]:
            self.c.execute(
                "INSERT INTO files(uploader,time,category,file_name,file)\
                     VALUES(?,?,?,?,?)",
                (uploader, date, file_category, file_name, file_url),
            )
            self.conn.commit()
            self.conn.close()
            return 1
    ...
    def file_retrieve(self, file_name: str, uploader: str):
        ...
        if file_name != "*":
            if uploader is not None:
                self.c.execute(
                    "SELECT COUNT(file_name) FROM files where file_name = ?\
                         AND uploader =?",
                    (
                        file_name,
                        uploader,
            ...
            if count == 1:
                if uploader is None:
                    ...
                    self.c.execute(
                        "SELECT * FROM files WHERE file_name = ? AND \
                            uploader =?",
                        (
                            file_name,
                            uploader,
                ...
                self.conn.close()
                return data
            elif count > 1 and uploader is None:
                return 2
            else:
                self.conn.close()
                return 0
            ...
            self.c.execute(
                # fmt: off
                "SELECT COUNT(file_name) FROM files where uploader = ?",
                (uploader,)
                # fmt: on
            )
            if self.c.fetchall()[0][0]:
                # fmt: off
                self.c.execute(
                    "SELECT * FROM files WHERE file_name = ?", (uploader,)
                    )
                # fmt: on
                data = self.c.fetchone()
                self.conn.close()
    ...
    def list_saved_files(self):
        ...
        if self.c.fetchall()[0][0] != 0:
            self.c.execute(
                "SELECT id,file_name,category,uploader,time\
                 FROM files"
            )
